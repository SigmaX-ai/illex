...
/**
 * \brief Look up Arrow field metadata value by key, and convert it to an uint64_t.
 * \param field The field to inspect.
 * \param key The key to look up.
...
class SchemaAnalyzer : public arrow::TypeVisitor {
 public:
  /// \brief Construct a SchemaAnalyzer, setting the DocumentGenerator to populate.
  explicit SchemaAnalyzer(DocumentGenerator* out);
  /// \brief Analyze an Arrow schema.
...
class FieldAnalyzer : public arrow::TypeVisitor {
 public:
  ...
  /// \brief Analyze an Field schema. This populates the Member generator supplied during
  /// construction.
...
/**
 * \brief Read an Arrow schema from file.
 * \param file The file containing the schema.
 * \param out A shared pointer to store the resulting schema in.
 * \return OK if successful, some error otherwise.
 *
 * Schema must be stored as raw output of Arrow's serialization functions.
 */
 ...
 * \return A DocumentGenerator able to generate JSON objects derived from the schema.
 */
auto FromArrowSchema(const arrow::Schema& schema,
                     GenerateOptions options = GenerateOptions()) -> DocumentGenerator;
